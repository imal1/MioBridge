...
import { KERNEL_TYPES, type ClusterStatus, type KernelDetection, type KernelType, type NodeKernelConfig } from '@/lib/types'
...
export default function RuntimesPage() {
  ...
  return (
    <SignalPage crumb="Runtime operations" title="运行时" description="维护 mihomo 与协议核心的运行状态、配置路径和 Agent 监控范围。" status={node ? `${node.name} · ${detections.filter(item => item.installed).length} 个协议核心已安装` : '请选择节点'} maxWidth="narrow" actions={<Button variant="outline" onClick={() => detect()} disabled={!nodeId || busy !== null}><Icon icon={busy?.endsWith(':detect') ? 'ph:spinner-bold' : 'ph:magnifying-glass-light'} className={busy?.endsWith(':detect') ? 'animate-spin' : ''} />检测运行时</Button>}>
      ...
      {error ? <Alert variant="destructive"><AlertTitle>运行时操作失败</AlertTitle><AlertDescription>{error}</AlertDescription></Alert> : null}
        ...
        {KERNEL_TYPES.map(type => {
          ...
          return (
            <Card key={type}>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-2 text-sm"><div className="flex justify-between gap-4"><span className="text-muted-foreground">二进制路径</span><code className="break-all text-right">/usr/local/bin/{type}</code></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">配置路径</span><code className="break-all text-right">{node.configuredKernels.find(item => item.type === type)?.configPath || detected?.defaultConfigPath || '-'}</code></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">可读状态</span><span>{runtime?.accessible ? '可读' : runtime?.error || '未知'}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">来源数量</span><span>{runtime?.nodesCount ?? '-'}</span></div></div>
      ...
      {editorOpen && node ? <KernelDetectionDialog open detections={detections} monitoredTypes={node.configuredKernels.map(item => item.type)} submitting={busy === `${node.nodeId}:monitoring`} error={error} confirmLabel="保存并验证监控配置" onCancel={() => setEditorOpen(false)} onConfirm={saveMonitoring} /> : null}
    </SignalPage>
  )
}
