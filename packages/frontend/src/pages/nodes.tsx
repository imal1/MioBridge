...
export default function NodesPage() {
  ...
  const nodes = useMemo(() => (cluster?.nodes || []).filter(node => {
    ...
    return !needle || [node.name, node.host, node.location, node.nodeId].some(value => value?.toLowerCase().includes(needle))
  ...
  const openEdit = (node: NodeStatus) => {
    setEditing(node)
    setDraft({
      name: node.name, host: node.host || '', location: node.location,
  ...
  return <SignalPage crumb="Node inventory" title="节点" description="管理节点档案、SSH 连接和纳管状态；软件安装与运行维护由后续页面负责。" status={`${cluster?.totalNodes || 0} 个节点档案`} maxWidth="narrow" actions={<><Button variant="outline" onClick={refresh}><Icon icon="ph:arrow-clockwise-light" />刷新</Button><Button onClick={() => setAddOpen(true)}><Icon icon="ph:plus-light" />添加节点</Button></>}>
    ...
    <Dialog open={Boolean(editing)} onOpenChange={open => { if (!open) setEditing(null) }}><DialogContent><DialogHeader><DialogTitle>编辑节点档案</DialogTitle><DialogDescription>{editing?.nodeId === 'local' ? '本机节点固定通过回环地址直接执行。' : '修改主机会清除已确认的 host key，下次部署前需要重新预检。'}</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><div className="grid gap-2"><Label htmlFor="edit-name">名称</Label><Input id="edit-name" value={draft.name} onChange={event => setDraft(previous => ({ ...previous, name: event.target.value }))} /></div><div className="grid gap-2"><Label htmlFor="edit-host">主机</Label><Input id="edit-host" value={draft.host} disabled={editing?.nodeId === 'local'} onChange={event => setDraft(previous => ({ ...previous, host: event.target.value }))} /></div><div className="grid gap-2"><Label htmlFor="edit-location">地域标签</Label><Input id="edit-location" value={draft.location} onChange={event => setDraft(previous => ({ ...previous, location: event.target.value }))} /></div>
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="grid gap-2"><Label htmlFor="edit-ssh-user">用户名</Label><Input id="edit-ssh-user" value={draft.sshUser} onChange={event => setDraft(previous => ({ ...previous, sshUser: event.target.value }))} /></div>
      ...
      <div className="grid gap-2">
        <Label htmlFor="edit-ssh-credential">{draft.sshAuthMethod === 'password' ? '密码' : '私钥'}</Label>
        <Input id="edit-ssh-credential" type="password" autoComplete="new-password" value={draft.sshCredential} onChange={event => setDraft(previous => ({ ...previous, sshCredential: event.target.value }))} placeholder={draft.sshUser.trim() === 'root' ? 'root 凭据仅供下一次部署使用，不保存' : '留空保留现有凭据；普通用户凭据默认保存'} />
      </div></> : <div className="grid gap-2"><Label htmlFor="edit-ssh-credential">密码</Label><Input id="edit-ssh-credential" type="password" autoComplete="new-password" value={draft.sshCredential} onChange={event => setDraft(previous => ({ ...previous, sshCredential: event.target.value }))} placeholder={draft.sshUser.trim() === 'root' ? 'root 密码仅供下一次部署使用，不保存' : '留空保留现有密码；普通用户密码默认保存'} /></div>}</div><DialogFooter><Button variant="outline" onClick={() => setEditing(null)}>取消</Button><Button onClick={saveEdit} disabled={Boolean(busy)}>保存档案</Button></DialogFooter></DialogContent></Dialog>
