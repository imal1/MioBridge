...
STANDALONE="$ROOT_DIR/packages/frontend/.next/standalone"
...
cp -r "$ROOT_DIR/packages/frontend/.next/static" "$RUNTIME/.next/static"
...
if [[ -d "$ROOT_DIR/packages/frontend/public" ]]; then
  cp -r "$ROOT_DIR/packages/frontend/public" "$RUNTIME/public"
fi
